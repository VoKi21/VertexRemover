...
public class ObjWriter {
    ...
    protected static StringBuilder getStringVerticesContent(List<Vector3f> vertices) {
        ...
        for (Vector3f v : vertices) {
            str.append(OBJ_VERTEX_TOKEN + " ").append(v.coordstoStringSplitBySpace()).append("\n");
        }
        str.append("# ").append(vertices.size()).append(" vertices\n");
        return str;
    ...
    protected static StringBuilder getStringTextureVerticesContent(List<Vector2f> textureVertices) {
        ...
        for (Vector2f vt : textureVertices) {
            str.append(OBJ_TEXTURE_TOKEN + " ").append(vt.coordstoStringSplitBySpace()).append("\n");
        }
        str.append("# ").append(textureVertices.size()).append(" texture coords\n");
        return str;
    }
    ...
    protected static StringBuilder getStringNormalsContent(List<Vector3f> normals) {
        ...
        for (Vector3f vn : normals) {
            str.append(OBJ_NORMAL_TOKEN + " ").append(vn.coordstoStringSplitBySpace()).append("\n");
        }
        str.append("# ").append(normals.size()).append(" vertex normals\n");
        return str;
    }
    ...
    protected static StringBuilder getStringPolygonsContent(List<Polygon> polygons) {
        ...
        for (Polygon polygon : polygons) {
            str.append(OBJ_FACE_TOKEN + " ").append(polygonInfoInObjForm(polygon)).append("\n");
        }
    ...
    private static StringBuilder polygonInfoString(Polygon polygon) {
        ...
        if (!textureVertexIndices.isEmpty()) {
            ...
            for (int i = 0; i < oneVertexInPolygonInfoString.size(); i++) {
                oneVertexInPolygonInfoString.get(i).append(slash).append(textureVertexIndices.get(i) + 1);
            }
        }
        if (!normalIndices.isEmpty()) {
            slash = (isTextured) ? "/" : "//";
            for (int i = 0; i < oneVertexInPolygonInfoString.size(); i++) {
                oneVertexInPolygonInfoString.get(i).append(slash).append(normalIndices.get(i) + 1);
            }
        }
    ...
    protected static File getObjFileByPath(String filePath) throws IOException {
        ...
        if (!Files.exists(path)) {
            filePath = (filePath.isEmpty()) ? "newFile.obj" : getFileName(filePath);
